...
class DCGAN:
	...
	def build(self):
		...
		self.lowers_mask = tf.placeholder(tf.float32, [8, 8, 3], name='lowres_mask')
	...
	def train_completion(self):
		...
		img = data_index(config.uncompletion_image_dir, shuffle=True)
		num_img = len(img)
		...
		# random to choose 64 images
		l = 0
		u = min(self.batch_size, num_img)
		batch_size_z = u - l
		batch_files = img[l : u]
